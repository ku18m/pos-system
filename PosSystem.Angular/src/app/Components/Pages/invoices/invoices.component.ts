...
export class InvoicesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.unitService
      .getAllUnits()
      .subscribe({ next: (response) => (this.unitList = response) });
    this.clientService
      .getAllClients()
      .subscribe({ next: (response) => (this.clientList = response) });
    this.itemService
      .getAllItems()
      .subscribe({ next: (response) => (this.itemList = response) });
    this.invoiceService.getAllBills().subscribe({
      next: (bills: IInvoices[]) => {
    ...
    this.invoiceService
      .getAllEmployees()
      .subscribe({ next: (response) => (this.employeeList = response) });
    this.invoiceService.getAllBillEmployees().subscribe({
      next: (bills: IInvoices[]) => {
  ...
  billSubmit(e: any) {
    ...
    this.itemList.forEach((item) => {
      if (item.name == this.itemName) {
        ...
        } else this.quantityGraterError = null;
      }
    });
    ...
    if (
      ...
      const newBill: any = {
        billsDate: this.billDate,
        billsNumber: this.billNumber,
  ...
  checkPercentage() {
    if (Number(this.percentage) < 0) {
      this.percentageError =
        'Percentage Discount Must Be Greater Than Or Equal Zero';
    } else this.percentageError = null;
  }

  checkValue() {
    if (Number(this.discountValue) < 0) {
      this.discountValueError =
        'Value Discount Must Be Greater Than Or Equal Zero';
    } else this.discountValueError = null;
  }
  ...
  employeeSubmit(e: any) {
    ...
    } else this.employeeNameError = null;
