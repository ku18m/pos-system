...
export class InvoicesComponent implements OnInit {
     ...
     next: (response) => {
       response.forEach((element: any) => {
         if (element.role == "Employee") {
           this.employeeList.push(element.firstName + " " + element.lastName)
         }
       });
     }
   });
  ...
  //  });
   ...
   this.billTableItem.forEach(bill => {
    tableCount++;
      ...
      console.log(`billCount= ${tableCount}`)
 ...
 FinalSubmit(e: any) {
   ...
   this.invoiceItems.push({ itemId: this.itemId, quantity: this.itemQuantity, unitId: this.unitId, sellingPrice: this.itemSellingPrice })
   //ADDING INVOICE
   if (this.billDate != null && this.clientName != "-- Choose From Clients --" && this.itemName != "-- Choose From Items --" && this.quantity != null && this.quantity != "" && this.quantity != 0 && Number(this.percentage) > 0 && Number(this.discountValue) > 0 && this.employeeName != "-- Choose From Employees --" && this.date != null) {
     this.invoiceService.addInvoice(this.date, this.billDate, this.paidUp, this.discountValue, this.net, this.invoiceItems, this.clientId, this.employeeId);
